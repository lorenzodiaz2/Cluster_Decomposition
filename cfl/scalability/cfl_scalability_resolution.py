...
def run_scalability (
    ...
    for n_quadrants in q_range:
        ...
        for i in range(n_iterations):
            ...
            print(f"n quadrants = {n_quadrants}    n clients per quadrant = {n * 5}    n facilities per quadrant = {n}    offset = {offset}    iteration {i}   ", end="")
            ...
            for cluster in env.clusters:
                hs = cluster_solver_maker(env, cluster)
                hs.solve()
                env.clusters_solvers.append(hs)
            ...
            save_cfl_results(env, critical_resources, final_solver, complete_solver, seed, df)
            seed += 1
            df.to_csv(f"results/output.csv", index=False)
        print()
...
def run_sscfl_scalability_instance(grid_side, n, max_cluster_size, offset, k, df, seed, q_range, n_iterations):
    ...
    run_scalability(grid_side, n, max_cluster_size, offset, k, df, seed, q_range, n_iterations, env_maker, complete_solver_maker, cluster_solver_maker, critical_resources_maker)
...
def run_mscfl_scalability_instance(grid_side, n, max_cluster_size, offset, k, df, seed, q_range, n_iterations):
    ...
    complete_solver_maker = lambda env, cr: MSCFL_Heuristic_solver(env.G, env.clients, env.facilities, cr)
    cluster_solver_maker = lambda env, cluster: MSCFL_Heuristic_solver(env.G, cluster.clients, cluster.facilities)
    critical_resources_maker = lambda env: MSCFL_Critical_Resources(env.clients, env.facilities)
